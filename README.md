...
### ⚡ One-Command Automated Setup (First-Time Devs)

Run the automated developer setup script. On Windows, it auto-installs missing prerequisites (Node.js LTS, pnpm, Git), compiles all monorepo packages, runs the invariant test suite, and **automatically launches the interactive Server Control Panel (`server.bat`)**:
...
- **Windows (PowerShell)**:
  ```powershell
  .\setup.ps1
  # or if script execution is restricted on your system:
...
### 🎛️ Server Control Panel (`server.bat` / `pnpm`)

**You do NOT have to run the setup script every time!** Once setup has finished, developers can start, stop, restart, check status, or open the browser directly using the interactive control panel:

- **Windows Interactive Menu (Double-click or CMD)**:
  ```cmd
  server.bat
  ```
  *(Displays an interactive menu with options to Start, Stop, Restart, Check Status, or Open Browser)*.

- **Direct Commands**:
  | Action | Windows CMD / Batch | PowerShell | Cross-Platform (pnpm) |
  | :--- | :--- | :--- | :--- |
  | **Interactive Menu** | `server.bat` | `.\server.bat` | — |
  | **Start & Open Browser** | `start.bat` | `.\start.ps1` | `pnpm start` |
  | **Stop Server (Kill Ports)** | `node scripts/start.mjs stop` | `node scripts/start.mjs stop` | `pnpm stop` |
  | **Restart Server** | `node scripts/start.mjs restart` | `node scripts/start.mjs restart` | `pnpm restart` |
  | **Check Server Status** | `node scripts/start.mjs status` | `node scripts/start.mjs status` | `pnpm status` |
...
Run both the NestJS API and the Next.js Ops Console:

**Option 1 — Server Control Panel (Interactive)**:
```cmd
server.bat
```

**Option 2 — One-Command Runner (with auto-browser launch)**:
```bash
pnpm start
