...
export class PersistenceService implements PersistenceAdapter, OnModuleInit {
  ...
  private passengersState: Passenger[] = [];
  private notificationsState: PassengerNotification[] = [];
  ...
  private seedInitialState(): void {
    ...
    this.passengersState = [
      // Passengers for Flight AA101 (fl_101)
      {
        id: "psg_101_1",
        name: "John Doe",
        ...
        flightId: "fl_101",
        seatNumber: "12A",
        bookingReference: "REF-AA101-01",
        connectingFlightId: "fl_404",
        secureToken: "tok_john_doe_101"
      },
      {
        id: "psg_101_2",
        name: "Sarah Smith",
        ...
        flightId: "fl_101",
        seatNumber: "14C",
        bookingReference: "REF-AA101-02",
        connectingFlightId: "fl_202",
        secureToken: "tok_sarah_smith_101"
      },
      {
        id: "psg_101_3",
        name: "Alex Johnson",
        ...
        flightId: "fl_101",
        seatNumber: "21B",
        bookingReference: "REF-AA101-03",
        connectingFlightId: null,
        secureToken: "tok_alex_johnson_101"
      },
      {
        id: "psg_101_4",
        name: "Maria Garcia",
        ...
        flightId: "fl_101",
        seatNumber: "05F",
        bookingReference: "REF-AA101-04",
        connectingFlightId: "fl_404",
        secureToken: "tok_maria_garcia_101"
      },
      {
        id: "psg_101_5",
        name: "David Chen",
        ...
        flightId: "fl_101",
        seatNumber: "18D",
        bookingReference: "REF-AA101-05",
        connectingFlightId: null,
        secureToken: "tok_david_chen_101"
      },

      // Passengers for Flight UA202 (fl_202)
      {
        id: "psg_202_1",
        name: "Emily Davis",
        ...
        flightId: "fl_202",
        seatNumber: "08A",
        bookingReference: "REF-UA202-01",
        connectingFlightId: null,
        secureToken: "tok_emily_davis_202"
      },
      {
        id: "psg_202_2",
        name: "Michael Brown",
        phoneNumber: "+13125550143",
        flightId: "fl_202",
        seatNumber: "10B",
        bookingReference: "REF-UA202-02",
        connectingFlightId: "fl_606",
        secureToken: "tok_michael_brown_202"
      },
      {
        id: "psg_202_3",
        name: "Lisa Wilson",
        ...
        flightId: "fl_202",
        seatNumber: "15C",
        bookingReference: "REF-UA202-03",
        connectingFlightId: null,
        secureToken: "tok_lisa_wilson_202"
      },

      // Passengers for Flight DL303 (fl_303)
      {
        id: "psg_303_1",
        name: "Robert Taylor",
        phoneNumber: "+14045550177",
        flightId: "fl_303",
        seatNumber: "03C",
        bookingReference: "REF-DL303-01",
        connectingFlightId: null,
        secureToken: "tok_robert_taylor_303"
      },

      // Passengers for Flight BA404 (fl_404)
      {
        id: "psg_404_1",
        name: "James Anderson",
        ...
        flightId: "fl_404",
        seatNumber: "02A",
        bookingReference: "REF-BA404-01",
        connectingFlightId: null,
        secureToken: "tok_james_anderson_404"
      }
    ];
  }
  ...
  // --- Passenger & Notification Persistence Methods ---

  public getPassengers(): Passenger[] {
    return this.passengersState.map((p) => ({ ...p }));
  }

  public getPassengersByFlightId(flightId: string): Passenger[] {
    return this.passengersState
      .filter((p) => p.flightId === flightId)
      .map((p) => ({ ...p }));
  }

  public getPassengerByToken(token: string): Passenger | undefined {
    const p = this.passengersState.find((p) => p.secureToken === token);
    return p ? { ...p } : undefined;
  }

  public saveNotification(notification: PassengerNotification): void {
    const existingIndex = this.notificationsState.findIndex((n) => n.id === notification.id);
    if (existingIndex >= 0) {
      this.notificationsState[existingIndex] = { ...notification };
    } else {
      this.notificationsState.unshift({ ...notification });
    }
  }

  public updateNotification(notification: PassengerNotification): void {
    this.saveNotification(notification);
  }

  public getNotificationById(id: string): PassengerNotification | undefined {
    const n = this.notificationsState.find((n) => n.id === id);
    return n ? { ...n } : undefined;
  }

  public getNotifications(filter?: { flightId?: string; status?: NotificationStatus }): PassengerNotification[] {
    let result = [...this.notificationsState];
    if (filter?.flightId) {
      result = result.filter((n) => n.flightId === filter.flightId);
    }
    if (filter?.status) {
  ...
  public getNotificationStats() {
    const total = this.notificationsState.length;
    const delivered = this.notificationsState.filter((n) => n.status === "DELIVERED").length;
    const sent = this.notificationsState.filter((n) => n.status === "SENT").length;
    const failed = this.notificationsState.filter((n) => n.status === "FAILED").length;
    const pending = this.notificationsState.filter((n) => n.status === "PENDING").length;

    return { total, delivered, sent, failed, pending };
  }
}
