...
describe("API Layer E2E: Delay Simulation, Locked Commit & WebSocket Stream", () => {
  ...
  it("completes full operational lifecycle: board load -> delay preview -> commit -> WebSocket broadcast", async () => {
    ...
    expect(commitRes.body.notificationsSentCount).toBeGreaterThan(0);
  ...
  it("provides notification stats, audit list, passenger manifest, and mobile portal APIs", async () => {
    // 1. Verify notification stats
    const statsRes = await request(httpServer).get("/notifications/stats").expect(200);
    expect(statsRes.body.total).toBeGreaterThan(0);
    expect(statsRes.body.delivered).toBeGreaterThan(0);

    // 2. Verify notifications list endpoint
    const notifsRes = await request(httpServer).get("/notifications").expect(200);
    expect(Array.isArray(notifsRes.body)).toBe(true);
    expect(notifsRes.body.length).toBeGreaterThan(0);

    const firstNotif = notifsRes.body[0];
    expect(firstNotif.passengerName).toBeDefined();
    expect(firstNotif.messageBody).toContain("SkyOps Alert");
    expect(firstNotif.secureToken).toBeDefined();

    // 3. Verify passenger manifest endpoint
    const manifestRes = await request(httpServer).get("/passengers/manifest/fl_101").expect(200);
    expect(manifestRes.body.flight.flightNumber).toBe("AA101");
    expect(manifestRes.body.passengers.length).toBeGreaterThan(0);

    // 4. Verify passenger portal mobile endpoint using token
    const portalRes = await request(httpServer).get(`/passenger/portal/${firstNotif.secureToken}`).expect(200);
    expect(portalRes.body.passenger.name).toBe(firstNotif.passengerName);
    expect(portalRes.body.flight).toBeDefined();
    expect(portalRes.body.notifications.length).toBeGreaterThan(0);
  });
