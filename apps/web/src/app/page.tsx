...
import { PassengerNotificationConsole } from "../components/notifications/PassengerNotificationConsole";
import { MissedConnectionAlertBanner } from "../components/notifications/MissedConnectionAlertBanner";
import { PassengerManifestModal } from "../components/notifications/PassengerManifestModal";
...
export default function OperationsConsolePage() {
  ...
  const [manifestFlightId, setManifestFlightId] = useState<string | null>(null);

  const selectedManifestFlight = board?.flights.find((f) => f.id === manifestFlightId);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-950">
      ...
      <main className="flex-1 p-6 space-y-6 max-w-[1600px] w-full mx-auto">
        ...
        {/* Missed-Connection Risk Alert Banner */}
        <MissedConnectionAlertBanner
          board={board}
          onOpenManifest={(flightId) => setManifestFlightId(flightId)}
        />
        ...
        {loading && !board ? (
          ...
          <div className="space-y-6">
            ...
            {/* Automated Passenger SMS & Notification Console */}
            <PassengerNotificationConsole
              onOpenManifest={(flightId) => setManifestFlightId(flightId)}
            />
          </div>
        )}
      ...
      {/* Passenger Manifest View Modal */}
      {manifestFlightId && selectedManifestFlight && (
        <PassengerManifestModal
          flightId={manifestFlightId}
          flightNumber={selectedManifestFlight.flightNumber}
